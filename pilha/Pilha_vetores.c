/****************************************************************** 
 * Nome: Pilha vetores                                            *
 * Descricao: Implementacao de pilha vetores. Esse codigo possui  *
...
int push(Pilha *pilha, int nTamanho, float nValor, char cor[TAM_COR], char marca[TAM_MARCA]) {
    ...
    nova->valor = nValor;
    strncpy(nova->cor, cor, TAM_COR - 1);
    nova->cor[TAM_COR - 1] = '\0'; // Garantir terminação com nulo
    strncpy(nova->marca, marca, TAM_MARCA - 1);
    nova->marca[TAM_MARCA - 1] = '\0'; // Garantir terminação com nulo
